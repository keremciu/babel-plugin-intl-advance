...
var _default = function _default(_ref) {
  ...
  var isFormatMessageCall = function(path, opts) {
    ...
    var formatMessageKeys = opts.formatMessageKeys || [
      'formatMessage',
      'formatHTMLMessage',
    ]
  ...
  return {
    ...
    post(file) {
      ...
      var basename = _path.default.basename(
        filename,
      ...
      if (opts.messagesDir && descriptors.length > 0) {
        ...
        var relativePath = _path.default.join(
          _path.default.sep,
          _path.default.relative(process.cwd(), filename),
        )

        var messagesFilename = _path.default.join(
          opts.messagesDir,
          _path.default.dirname(relativePath),
          `${basename}.json`,
        )
    ...
    visitor: {
      CallExpression(path, state) {
        ...
        var processTranslationKey = function(translationObject) {
          ...
          var idProperty = properties.find(function(prop) {
            return prop.get('key').name === 'id'
          })
          var idValue = evaluatePath(idProperty.get('value')).trim()
          storeTranslationKey(idValue, state) // Tag the AST node so we don't try to extract it twice.
